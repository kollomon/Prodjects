...
namespace XGS_ENGINE
{
	...
	public:
		...
		__declspec(dllexport) void loadSprite(string dir, int W, int H);
		...
		__declspec(dllexport) void VectorEN_VectorMove(float tempX, float tempY);
	...
	private:
		...
		string _stop = ";";
		char stop = _stop[0];
		__declspec(dllexport) bool checkString(std::string find);

		__declspec(dllexport) int getID(std::string find);

		string name_;
	public:

		__declspec(dllexport) bool openOnWriteXGSDocument(std::string dir);

		__declspec(dllexport) bool openOnReadXGSDocument(std::string dir);

		__declspec(dllexport) void write(string _name, string value);

		__declspec(dllexport) vector<char> read(std::string name);

		__declspec(dllexport) void exit();

		__declspec(dllexport) int getTypeInt(vector <char> simvols);
